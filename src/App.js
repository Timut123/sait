...
import { 
  ...
  Car, Truck, Paperclip, Bike, Trophy, MapPin, AlertCircle, FileText, User
} from 'lucide-react';
...
const LOGO_URL = "5462928930428882216 (1).jpg"; // Убедитесь, что файл с этим именем лежит в папке public
...
const HomePage = () => (
  <div className="w-full">
    {/* HERO */}
    <div className="bg-[#FFD600] flex flex-col justify-center items-center relative container mx-auto px-4 md:px-8 pt-10 pb-20">
      <div className="w-full flex flex-col lg:flex-row items-center justify-between gap-8">
        <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto z-20">
          <a href={`tel:${PHONE_CLEAN}`} className="bg-[#32353C] hover:bg-black text-white px-8 py-3 rounded-lg font-medium text-lg shadow-xl transition text-center min-w-[150px]">Позвонить</a>
          <a href={REGISTRATION_LINK} target="_blank" rel="noreferrer" className="bg-[#00D756] hover:bg-[#00c04b] text-white px-8 py-3 rounded-lg font-medium text-lg shadow-xl transition flex items-center justify-center gap-2 min-w-[220px]">Стать Водителем <WhatsappIcon /></a>
        </div>
        <div className="relative z-10 w-full max-w-[800px] flex justify-center lg:justify-end">
            <img src="https://taxipark.kz/wp-content/uploads/2021/04/optim_yandex_taxi_car.png" alt="Yandex Taxi Yellow Car" className="w-full object-contain drop-shadow-2xl" onError={(e) => { e.target.style.display = 'none'; }} />
        </div>
      </div>
      <h1 className="text-3xl md:text-5xl lg:text-[52px] font-bold text-center leading-tight mt-12 text-[#1d1d1d] max-w-5xl">Станьте водителем Яндекс такси по Казахстану</h1>
    </div>

    {/* ПРЕИМУЩЕСТВА */}
    <section className="py-24 bg-white">
      <div className="container mx-auto px-4 md:px-8">
        <div className="grid md:grid-cols-3 gap-16 md:gap-8 text-left">
          {[
            {icon: Phone, title: "Быстрое подключение", desc: "Подключим Вас к Яндекс такси с лучшими условиями по всему Казахстану за кратчайшие сроки!"},
            {icon: Smartphone, title: "Удобное приложение", desc: "Даже если у Вас нет опыта работы вы быстро научитесь, благодаря удобному интерфейсу."},
            {icon: Clock, title: "Круглосуточная поддержка", desc: "Если у Вас возникнут вопросы по работе, Вы в любое время можете написать в тех. поддержку."}
          ].map((item, idx) => (
             <div key={idx}>
                <div className="mb-6 relative h-20 w-20">
                    <div className="absolute inset-0 border-2 border-[#FFD600] rounded-2xl -rotate-6"></div>
                    <div className="absolute inset-0 border-2 border-[#FFD600] rounded-2xl rotate-0 bg-white flex items-center justify-center z-10">
                      <div className="text-[#FFD600]"><item.icon size={36} /></div>
                    </div>
                </div>
                <h3 className="text-2xl font-bold mb-4 text-[#1d1d1d]">{item.title}</h3>
    ...
    <div className="container mx-auto px-4 md:px-8 py-16 grid lg:grid-cols-2 gap-12 items-center relative z-10">
      ...
      <div className="text-[#1d1d1d]">
        ...
        <p className="mt-10 text-lg opacity-80">
          Приходите регистрацию и начните зарабатывать с лучшим таксопарком в регионе.
        </p>
      </div>

      {/* Правая колонка - Картинка */}
      <div className="relative flex justify-center lg:justify-end mt-8 lg:mt-0">
         {/* Имитация плашки как на фото */}
         ...
         <img 
           ...
           style={{clipPath: "polygon(10% 0, 100% 0, 100% 100%, 0% 100%)"}} // Небольшой скос как стилизация
         />
      </div>
    </div>
  </div>
...
// 5. СТРАНИЦА: КОНТАКТЫ (НОВАЯ)
const ContactsPage = () => (
  <div className="w-full bg-white flex-grow animate-fade-in">
    {/* Желтый заголовок на всю ширину (Опционально, если нужно как на скрине Header) */}
    {/* Но судя по скрину контактов - там просто белый фон и заголовок */}
    ...
    {/* Темный футер уже есть в App, поэтому здесь просто контент контактов */}
  </div>
);
...
export default function App() {
  ...
  const [isAboutDropdownOpen, setIsAboutDropdownOpen] = useState(false); // Новое состояние для меню "О нас"
    ...
    <div className="font-sans antialiased text-[#1d1d1d] bg-white min-h-screen flex flex-col">
      ...
      <div className="bg-[#FFD600]">
        <header className="w-full py-6 z-50">
          <div className="container mx-auto px-4 md:px-8 flex items-center justify-between">
            <div className="flex-shrink-0 cursor-pointer" onClick={() => setCurrentView('home')}>
                ...
                <img 
                  src={LOGO_URL} 
                  alt="Logo" 
                  className="w-16 h-16 rounded-full border-2 border-yellow-200 object-cover shadow-lg"
                  onError={(e) => {
            ...
            <nav className="hidden lg:flex items-center gap-10 font-semibold text-[15px] text-[#1d1d1d]">
              {/* Dropdown "Работа" */}
            ...
            <div className="hidden lg:flex items-center gap-6">
              <a href={`tel:${PHONE_CLEAN}`} className="flex items-center gap-2 font-semibold text-lg hover:opacity-70 transition"><WhatsappIcon /> {PHONE_DISPLAY}</a>
          ...
          {/* Mobile Menu */}
          {isMenuOpen && (
            <div className="lg:hidden absolute top-24 left-0 w-full bg-[#FFD600] shadow-2xl p-6 z-50 border-t border-black/5">
